...
// El Handler:
public class UpdateNotePositionCommandHandler(ApplicationDbContext context)
    : IRequestHandler<UpdateNotePositionCommand>
{
    public async Task Handle(UpdateNotePositionCommand request, CancellationToken cancellationToken)
    {
        ...
        if (note is not null)
        {
            note.X = request.Dto.X;
            note.Y = request.Dto.Y;
